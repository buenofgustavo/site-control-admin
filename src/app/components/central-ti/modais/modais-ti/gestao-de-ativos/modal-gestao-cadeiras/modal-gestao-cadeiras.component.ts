...
@Component({
  selector: 'app-modal-gestao-cadeiras',
  templateUrl: './modal-gestao-cadeiras.component.html',
  styleUrls: ['./modal-gestao-cadeiras.component.scss']
})
export class ModalGestaoCadeirasComponent {
  selectedItemCategoria = "Cadeiras"
    ...
    this.gestaoAtivosService.cadastrarAtivos(this.gestaoAtivos).subscribe(
      ...
      setTimeout(() => {
        this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
          this.router.navigate(['/gestao-cadeiras-ti']); // Navega para a rota de cadastro de armazém
        });
        }, 1000); 
      },
      error =>{
