...
@Component({
  selector: 'app-modal-gestao-transformador',
  templateUrl: './modal-gestao-transformador.component.html',
  styleUrls: ['./modal-gestao-transformador.component.scss']
})
export class ModalGestaoTransformadorComponent {
  selectedItemCategoria = "Transformador"
    ...
    this.gestaoAtivosService.cadastrarAtivos(this.gestaoAtivos).subscribe(
      ...
      setTimeout(() => {
        this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
          this.router.navigate(['/gestao-transformadores-ti']); // Navega para a rota de cadastro de armazém
        });
        }, 1000); 
      },
      error =>{
