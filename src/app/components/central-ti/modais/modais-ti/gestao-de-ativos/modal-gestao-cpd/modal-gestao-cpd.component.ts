...
@Component({
  selector: 'app-modal-gestao-cpd',
  templateUrl: './modal-gestao-cpd.component.html',
  styleUrls: ['./modal-gestao-cpd.component.scss']
})
export class ModalGestaoCpdComponent {
    ...
    this.gestaoAtivosService.cadastrarAtivos(this.gestaoAtivos).subscribe(
      ...
      setTimeout(() => {
        this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
          this.router.navigate(['/gestao-cpd-ti']); // Navega para a rota de cadastro de armazém
        });
        }, 1000); 
      },
      error =>{
