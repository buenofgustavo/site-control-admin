...
@Component({
  selector: 'app-modal-gestao-bebedouros',
  templateUrl: './modal-gestao-bebedouros.component.html',
  styleUrls: ['./modal-gestao-bebedouros.component.scss']
})
export class ModalGestaoBebedourosComponent {
  selectedItemCategoria = "Bebedouro"
    ...
    this.gestaoAtivosService.cadastrarAtivos(this.gestaoAtivos).subscribe(
      ...
      setTimeout(() => {
        this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
          this.router.navigate(['/gestao-bebedouros-ti']); // Navega para a rota de cadastro de armazém
        });
        }, 1000); 
      },
      error =>{
