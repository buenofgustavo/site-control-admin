...
@Component({
  selector: 'app-modal-gestao-armarios',
  templateUrl: './modal-gestao-armarios.component.html',
  styleUrls: ['./modal-gestao-armarios.component.scss']
})
export class ModalGestaoArmariosComponent {
  selectedItemCategoria = "Armario"
    ...
    this.gestaoAtivosService.cadastrarAtivos(this.gestaoAtivos).subscribe(
      ...
      setTimeout(() => {
        this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
          this.router.navigate(['/gestao-armarios-ti']); // Navega para a rota de cadastro de armazém
        });
        }, 1000); 
      },
      error =>{
